...
namespace TestNonCoreWebApp.Helpers
{
    public class PwnedRestClient
    {
        ...
        public PwnedRestClient(string userAgent = null)
        {
        ...
        public async Task<bool> IsPasswordPwned(string password)
        {
            ...
            var responseString = GETRequest($"{api}/{hashFirstFive}", passwordRangeURL);
            ...
            var responseContainsHash = responseString.Contains(hashLeftover);
        ...
        private String GETRequest(string parameters, string overrideURL)
        {
            string result = string.Empty;
            using (var client = new HttpClient(new HttpClientHandler { AutomaticDecompression = DecompressionMethods.GZip | DecompressionMethods.Deflate }))
            {
                //client.BaseAddress = new Uri("https://api.stackexchange.com/2.2/");
                HttpResponseMessage response = client.GetAsync($"{overrideURL}/{parameters}").Result;
                response.EnsureSuccessStatusCode();
                result = response.Content.ReadAsStringAsync().Result; 
                
            }
            //string response = await GetAsync($"{overrideURL}/{parameters}");
